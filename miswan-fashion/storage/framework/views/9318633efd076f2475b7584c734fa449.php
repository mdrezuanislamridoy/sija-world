<?php $__env->startSection('title', $product->name . ' - ' . ($globalSetting->site_name ?? 'Miswan Fashion')); ?>
        ...
        <nav aria-label="breadcrumb" class="mb-3">
            <ol class="breadcrumb bg-transparent p-0">
                ...
                <?php if($product->category): ?>
                    <li class="breadcrumb-item"><a href="<?php echo e(route('category.show', $product->category->slug)); ?>"><?php echo e($product->category->name); ?></a></li>
                <?php endif; ?>
            ...
            <div class="row">
                ...
                <div class="col-lg-5 col-md-6 mb-4 mb-md-0">
                    <div class="product-gallery">
                        ...
                        <div class="main-image-wrapper border rounded overflow-hidden mb-2 text-center" style="height: 380px;">
                            <img id="mainProductImg" src="<?php echo e(asset($product->thumbnail)); ?>" class="img-fluid h-100" style="object-fit: contain;" alt="<?php echo e($product->name); ?>">
                        </div>

                        <!-- Thumbnails -->
                        <?php if($product->galleries && $product->galleries->count() > 0): ?>
                            <div class="thumbnails-row d-flex gap-2">
                                <div class="thumb-item border rounded p-1 mr-2 cursor-pointer active" onclick="switchImage('<?php echo e(asset($product->thumbnail)); ?>', this)" style="width: 70px; height: 70px; cursor: pointer;">
                                    <img src="<?php echo e(asset($product->thumbnail)); ?>" class="img-fluid w-100 h-100" style="object-fit: cover;">
                                </div>
                                <?php $__currentLoopData = $product->galleries; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $gallery): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div class="thumb-item border rounded p-1 mr-2 cursor-pointer" onclick="switchImage('<?php echo e(asset($gallery->image)); ?>', this)" style="width: 70px; height: 70px; cursor: pointer;">
                                        <img src="<?php echo e(asset($gallery->image)); ?>" class="img-fluid w-100 h-100" style="object-fit: cover;">
                                    </div>
                ...
                <div class="col-lg-7 col-md-6">
                    <div class="product-info-content pl-lg-3">
                        <span class="badge badge-pill badge-primary mb-2"><?php echo e($product->category->name ?? 'Collection'); ?></span>
                        <h1 class="font-weight-bold text-dark mb-2" style="font-size: 24px;"><?php echo e($product->name); ?></h1>
                        <p class="text-muted small mb-2">SKU: <strong class="text-dark"><?php echo e($product->sku ?? 'MSW-PROD-' . $product->id); ?></strong></p>
                        ...
                        <div class="price-box d-flex align-items-center mb-3 bg-light p-3 rounded">
                            <span class="h3 font-weight-bold text-primary m-0 mr-3" id="displayProductPrice">
                                <?php echo e($globalSetting->currency_symbol ?? 'TK'); ?> <?php echo e(number_format($product->price)); ?>

                            </span>
                            <?php if($product->previous_price && $product->previous_price > $product->price): ?>
                                <span class="text-muted h5 m-0 mr-3 text-decoration-line-through">
                                    <del><?php echo e($globalSetting->currency_symbol ?? 'TK'); ?> <?php echo e(number_format($product->previous_price)); ?></del>
                                </span>
                                <span class="badge badge-danger font-weight-bold p-2" style="font-size: 13px;">-<?php echo e($product->discount_percent); ?>% Discount</span>
                            <?php endif; ?>
                        </div>
                        ...
                        <div class="stock-status-box mb-3">
                            <span class="text-success font-weight-bold" id="displayProductStock">
                                <i class="fa fa-check-circle mr-1"></i> Stock Remaining: <?php echo e(number_format($product->stock)); ?> Items
                            </span>
                        </div>
                        ...
                        <!-- Product Form -->
                        <form method="POST" action="<?php echo e(route('cart.add')); ?>" id="productPurchaseForm">
                            <?php echo csrf_field(); ?>
                            <input type="hidden" name="product_id" value="<?php echo e($product->id); ?>">
                            <input type="hidden" name="variant_id" id="selected_variant_id" value="<?php echo e($product->variants->first()->id ?? ''); ?>">
                                ...
                                <?php if($product->variants && $product->variants->count() > 0): ?>
                                    <div class="variant-buttons d-flex flex-wrap gap-2">
                                        <?php $__currentLoopData = $product->variants; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $variant): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                            <button type="button" 
                                                    class="btn btn-outline-dark mr-2 mb-2 variant-pill <?php echo e($index == 0 ? 'active' : ''); ?>" 
                                                    data-variant-id="<?php echo e($variant->id); ?>"
                                                    data-price="<?php echo e($variant->price ?? $product->price); ?>"
                                                    data-stock="<?php echo e($variant->stock); ?>"
                                                    data-image="<?php echo e($variant->image ? asset($variant->image) : ''); ?>"
                            ...
                            <div class="color-selection mb-4">
                                ...
                                <select name="color" class="form-control" style="max-width: 200px; border-radius: 8px;" required>
                                    ...
                                    <?php if(!empty($product->colors)): ?>
                                        <?php $colorList = array_map('trim', explode(',', $product->colors)); ?>
                            ...
                            <div class="purchase-action-row d-flex flex-wrap align-items-center mb-4">
                                <div class="quantity-input-group d-flex justify-content-between align-items-center border rounded mr-3 mb-2 bg-light" style="width: 130px; height: 45px; overflow: hidden;">
                                    ...
                                    <input type="text" id="product_quantity" name="quantity" value="1" min="1" max="<?php echo e($product->stock); ?>" class="text-center border-0 font-weight-bold m-0 p-0" style="width: 40px; background: transparent; outline: none; box-shadow: none; font-size: 16px;" readonly>
                                ...
                                <button type="button" class="btn btn-outline-primary h-100 px-4 mr-2 mb-2 ajax-add-to-cart" data-product-id="<?php echo e($product->id); ?>" style="height: 45px;">
                ...
                <?php $__currentLoopData = $relatedProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $rel): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <div class="col-6 col-md-3 mb-4">
                        <div class="product border rounded bg-white p-2 h-100 shadow-sm">
                            <a href="<?php echo e(route('product.show', ['slug' => $rel->slug, 'id' => $rel->id])); ?>" class="text-decoration-none">
                                <div class="thumb rounded overflow-hidden mb-2" style="height: 180px;">
                                    <img src="<?php echo e(asset($rel->thumbnail)); ?>" class="img-fluid w-100 h-100" style="object-fit: cover;">
                                </div>
                                <h6 class="title text-center text-dark text-truncate mb-1"><?php echo e($rel->name); ?></h6>
                                <p class="text-center font-weight-bold text-primary m-0"><?php echo e($globalSetting->currency_symbol ?? 'TK'); ?> <?php echo e(number_format($rel->price)); ?></p>
                            </a>
                        </div>
                    </div>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\regin\Downloads\miswan-fashion\miswan-fashion\resources\views/frontend/product_details.blade.php ENDPATH**/ ?>